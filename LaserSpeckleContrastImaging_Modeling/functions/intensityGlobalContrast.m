function [K1,K2,KnoNoise,KnoNoisenodisc] = intensityGlobalContrast(images,stdParenchyma, parenchymaK)
...
for i = 1:length(intensityValues)
    ...
    KnoNoise(i) = globalContrast(normO20(:,:,specklesizechoice));
    KnoNoisenodisc(i) = globalContrast(normO20nodisc(:,:,specklesizechoice));
    for n = 1:noiseiterations
            K1(n,i) = globalContrast(normO20(:,:,specklesizechoice)+noiseI(:,:,n));
            K2(n,i) = globalContrast(normO20(:,:,specklesizechoice)+noiseI2(:,:,n));
%             %with Crosstalk
%             K1(n,i) = globalContrast(imgaussfilt(normO20(:,:,specklesizechoice),0.5,'FilterSize',7)+noiseI(:,:,n));
%             K2(n,i) = globalContrast(imgaussfilt(normO20(:,:,specklesizechoice),0.5,'FilterSize',7)+noiseI2(:,:,n));
